...
class ToolView(View):
    ...
    async def search_button(self, interaction: discord.Interaction, button: Button):
        ...
        if channel: await channel.send(f"!TOOL_REQUEST search_web \"{self.last_prompt}\" {self.user_id}")
    ...
    async def reimage_button(self, interaction: discord.Interaction, button: Button):
        ...
        channel = interaction.client.get_channel(self.channel_id)
        if channel: await channel.send(f"!TOOL_REQUEST generate_image \"{self.last_prompt}\" {self.user_id}")
    ...
    async def summarize_button(self, interaction: discord.Interaction, button: Button):
        ...
        channel = interaction.client.get_channel(self.channel_id)
        if channel: await channel.send(f"!TOOL_REQUEST summarize_last_message \"{self.last_prompt}\" {self.user_id}")
